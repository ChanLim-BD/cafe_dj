    ...
    def post(self, request):
        ...
        try:
            ...
            if data['phone'] == '' or (phone_regex.match(str(data['phone'])) is None):
                return Response({'MESSAGE': 'INVALID_PHONE_NUMBER'}, status=status.HTTP_400_BAD_REQUEST)
            
            # 암호가 제공되었는지 확인
            if data['password'] == '':
                return Response({'MESSAGE': 'PASSWORD_IS_REQUIRED'}, status=status.HTTP_400_BAD_REQUEST)

            # 중복된 전화 번호 확인
            if Account.objects.filter(phone=data['phone']).exists() and (data['phone'] != ''):
                return Response({'MESSAGE': 'PHONE_DUPLICATED'}, status=status.HTTP_400_BAD_REQUEST)

            # 암호 길이 확인
            if (len(data['password']) < 8):
                return Response({'MESSAGE': 'PASSWORD_VALIDATION'}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def post(self, request):
        ...
        try:
            ...
            if account == '' or password == '':
                return Response({'MESSAGE':'Enter Your User Phone or Password'}, status=status.HTTP_400_BAD_REQUEST)
            ...
            if Account.objects.filter(phone = account).exists():
                ...
                if user is not None:
                    ...
                    return Response({'MESSAGE':'SUCCESS', 'Authorization':access_token}, status=status.HTTP_200_OK)
                else:
                    return Response({'MESSAGE':'INVALID_USER'}, status=status.HTTP_401_UNAUTHORIZED)
